...
public class User {
    ...
    @Column(name = "nutrient_modelname")
    private String nutrientModelname;

    @Column(name = "nutrition_level")
    private Float nutritionLevel;

    @OneToMany(mappedBy = "user", fetch = FetchType.LAZY)
    private List<NutrientWeight> nutrientWeights = new ArrayList<>(); // 유저가 설정한 가중치들
    ...
    public void addMealInfo(MealRecord mealRecord) {
        this.mealRecords.add(mealRecord);       // for 자바내부객체 컬렉션 정합성 (실제론 userentity에 mealinfo정보는 없음)
        mealRecord.setUser(this);             // 관계성의 주인은 mealinfo -> user entity 참조를 추가함
    }

    public void removeMealInfo(MealRecord mealRecord) {
        this.mealRecords.remove(mealRecord);
        mealRecord.setUser(null);
    }

    public void addNutriWeight(NutrientWeight nutrientWeight) {
        this.nutrientWeights.add(nutrientWeight);
        nutrientWeight.setUser(this);
    }

    public void removeNutriWeight(NutrientWeight nutrientWeight) {
        this.nutrientWeights.remove(nutrientWeight);
    }
